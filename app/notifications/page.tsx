...
export default async function Notifications({ searchParams }: { searchParams: Promise<{ page?: string }> }) {
  ...
  return <main className="project-page">
    ...
    <div className="pagination-summary">Showing {total ? offset + 1 : 0}–{Math.min(offset + PAGE_SIZE, total)} of {total} deliveries</div>
    ...
    {totalPages > 1 && <nav className="pagination" aria-label="Notification pages">
      {page > 1 ? <Link href={`/notifications?page=${page - 1}`} rel="prev">← Previous</Link> : <span className="disabled">← Previous</span>}
      <div>{pages.map((number) => <Link key={number} href={`/notifications?page=${number}`} className={number === page ? "current" : ""} aria-current={number === page ? "page" : undefined}>{number}</Link>)}</div>
      {page < totalPages ? <Link href={`/notifications?page=${page + 1}`} rel="next">Next →</Link> : <span className="disabled">Next →</span>}
    </nav>}
  </main>;
}
